...
Vignette: [Using the package skeleton for patient-level prediction studies](https://raw.githubusercontent.com/OHDSI/StudyProtocolSandbox/master/RehospitalizationPredictionWithNote/inst/doc/UsingSkeletonPackage.pdf)

Instructions To Prepare Package Outside Atlas
===================

- Step 1: Change package name, readme and description (replace (RehospitalizationPredictionWithNote with the package name)
...
- Share the package by adding it to the OHDSI/StudyProtocolSandbox github repo and get people to install by running but replace 'RehospitalizationPredictionWithNote' with your study name if not using atlas:
```r
  ...
  devtools::install_github("OHDSI/StudyProtocolSandbox/RehospitalizationPredictionWithNote")
```

- Get users to execute the study by running the code in (extras/CodeToRun.R) but replace 'RehospitalizationPredictionWithNote' with your study name:
```r
